...
const Swap = () => {
  ...
  const fetchSwapAds = async () => {
    ...
    try {
      if (!auth.currentUser) {
        setError("You must be logged in to view swap listings.");
        setLoading(false);
        return;
      }
      ...
      if (querySnapshot.empty) {
        setError("No swap listings available at the moment.");
      }
  ...
  const filteredAds = swapAds.filter((ad) =>
    ...
    <div className="min-h-screen bg-gradient-to-r from-blue-50 to-purple-50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="container mx-auto">
        <div className="text-center mb-12">
          <h2 className="text-4xl font-extrabold text-gray-900 mb-4">Discover Swap Opportunities</h2>
          <p className="text-xl text-gray-600">Find great items to swap with our community</p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center mb-8 space-y-4 sm:space-y-0">
          ...
          >
            ...
            <option value="priceLowToHigh">Value: Low to High</option>
            <option value="priceHighToLow">Value: High to Low</option>
          </select>
        </div>
